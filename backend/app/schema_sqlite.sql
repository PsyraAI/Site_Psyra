-- Psyra AI — schema SQLite (espelho do schema_psyra_supabase.sql, S5)
-- Sprint: S6 | Risco: R2 (LGPD) — nenhuma coluna de PII de colaborador.
-- Paridade com Postgres/Supabase: mesmas 7 tabelas, mesmos CHECKs, mesma view agregada.
...
-- 4) ghe — Grupo Homogêneo de Exposição (NR-1) ---------------------------
...
-- 6) resposta — anônima. Sem nome, sem e-mail, sem matrícula. -------------
--    trava de conformidade: texto só entra marcado como anonimizado.
...
-- 7) resultado_ghe — agregado calculado (nunca individual) ---------------
...
-- 8) log_auditoria — cadeia de hash SHA-256 (LGPD Art. 37) ---------------
...
-- View de leitura do painel: k-anonimato n>=5 aplicado no próprio banco.
-- Equivalente à vw_painel_ghe do Supabase (lá com RLS por empresa).
CREATE VIEW IF NOT EXISTS vw_painel_ghe AS
SELECT
